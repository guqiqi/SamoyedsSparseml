...
class BF16OBS24pairPruningModifier(BaseGradualPruningModifier):
    ...
    def initialize(
        ...
        **kwargs,
        ...
        """
        _LOGGER.debug("Initializing BF16OBS24pairPruningModifier")
        if (
...
class BF16OBS24pairPruningParamsScorer(PruningParamsGradScorer):
    ...
    def score_parameters(self) -> List[Tensor]:
        ...
        m=2; n=1
        v=1
        _LOGGER.debug("_last_applied_sparsity "+ str(nn_row) + " " + str(mm_row) + " " + str(sparsity))
    ...
    def mask_update(self, masks: List[Tensor], mask_diffs: List[Tensor]):
        ...
        #     param_filename = './output/param_after_24_update_' + str(time_s) + '.csv'
