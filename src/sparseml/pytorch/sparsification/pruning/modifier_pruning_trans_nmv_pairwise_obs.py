...
__all__ = [
    "OBSTransnmvpairPruningModifier",
    "OBSTransnmvpairPruningParamsScorer",
]
...
@PyTorchModifierYAML()
class OBSTransnmvpairPruningModifier(BaseGradualPruningModifier):
    """
    ...
    def initialize(
        ...
        **kwargs,
        ...
        """
        _LOGGER.info("Initializing OBSTransnmvpairPruningModifier")
        if (
    ...
    def _get_scorer(self, params: List[Parameter]) -> PruningParamsGradScorer:
        ...
        """
        return OBSTransnmvpairPruningParamsScorer(
            params=params,
...
class OBSTransnmvpairPruningParamsScorer(PruningParamsGradScorer):
    """
    ...
    def score_parameters(self) -> List[Tensor]:
        ...
        nn_row, mm_row, v = self._mask_type.split(":")
        mm_row=int(mm_row); nn_row=mm_row-int(nn_row); v=int(v)
        m=2; n=1
        # v=128
        ...
        if self._is_main_proc:
            for i, finv in enumerate(self._finvs):
                ...
                exclude_columns_a, exclude_columns_b = self.find_best_combination0(sets, mm_row, nn_row, tensor_shape, "cpu", block_finv_w_pair.dtype)
                ...
                scores0[rows_b,cols_b*2+plus] = -1.0
                ...
                # TODO block_finv_w0需要转置回去
                block_finv_w[i] = block_finv_w0.t().to(self._devices[i])
                ...
                # TODO score转置回去再broadcast
                # scores[i] = scores0.t().reshape(self._params[i].shape).to(self._devices[i])
                scores[i] = scores0.reshape(trans_params.shape).t().to(self._devices[i])
    ...
    def pre_optim_step_update(self, masks: List[Tensor]):
        ...
        # TODO 这边应该要做转置, self._params[i].grad转置
        for i, finv in enumerate(self._finvs):
    ...
    def mask_update(self, masks: List[Tensor], mask_diffs: List[Tensor]):
            ...
            for i, param in enumerate(self._params):
                ...
                obs_updates[i] = (
                        ...
                        .mul(
                            ...
                            * (mask_diffs[i].to(self._devices[i]) == -1).reshape(-1).to(self._devices[i])
                        )
        ...
        #     param_filename = './output/param_after_12_trans_update_' + str(time_s) + '.csv'
        #     _LOGGER.info("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Saving log " + param_filename + " !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        #     np.savetxt(param_filename, self._params[0].detach().cpu().numpy(), delimiter=',', fmt='%1.2f')
